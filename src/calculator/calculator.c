...
float getFullNumber(char *str, int pos, calculatorErr *error) {
  ...
  while (isNumber(str[pos]) && str[pos] != '\0') {
    ...
    if (bufferIndex >= initialBufferSize) {
      ...
      if (!buffer) {
        setError(error,
                 "getFullNumber: error, could not increase memory buffer");
        free(buffer);
        return -1;
      }
    }
...
float add(float a, float b, calculatorErr *error) { return a + b; }

float sub(float a, float b, calculatorErr *error) { return a - b; }

float mul(float a, float b, calculatorErr *error) { return a * b; }
...
float mod(float a, float b, calculatorErr *error) { return (int)a % (int)b; }
...
operation_t parseOperator(char operator, calculatorErr * error) {
  ...
  default:
    setError(error,
             "parseOperator: error, %c is a non-valid operator", operator);
    break;
  }
  return operation;
}
...
char *formatOperation(char *str, calculatorErr *error) {
  ...
  if (!buffer) {
    setError(error,
             "formatOperation: error, could not allocate initial buffer");
    return NULL;
  }
...
void buildBinTree(char *str, operationNode **nodeRoot, calculatorErr *error) {
  ...
  for (int i = 0; i < size; i++) {
    if (str[i] == '(') {
      ...
      if (i - 1 < 0) {
        setError(error,
                 "buildBinTree: error, invalid expression: %s, started with "
                 "operator",
                 str);
        return;
      }
      ...
      if (i + 1 >= size) {
        setError(
            error,
            "buildBinTree: error, invalid expression: %s, ended in operator",
            str);
        return;
      }
      ...
      *nodeRoot = malloc(sizeof(operationNode));
      if (!*nodeRoot) {
        setError(error,
                 "buildBinTree: error, could not allocate memory for node");
        return;
      }
      ...
      if (isAtomic(firstHalf)) {
        ...
        if (error->raised) {
          setError(error,
                   "buildBinTree: error, could not determine atomicA: %s",
                   firstHalf);
          free(*nodeRoot);
          free(firstHalf);
          free(secondHalf);
        ...
        printf("%s\n", unwrappedFirstHalf);
        buildBinTree(unwrappedFirstHalf, &((*nodeRoot)->operandExpressionA),
                     error);
      ...
      if (isAtomic(secondHalf)) {
        ...
        if (error->raised) {
          setError(error,
                   "buildBinTree: error, could not determine atomicB: %s",
                   secondHalf);
          free(*nodeRoot);
          free(firstHalf);
          free(secondHalf);
        ...
        printf("%s\n", unwrappedsecondHalf);
        buildBinTree(unwrappedsecondHalf, &((*nodeRoot)->operandExpressionB),
                     error);
